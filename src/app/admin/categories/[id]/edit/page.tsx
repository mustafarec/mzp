...
import { ArrowLeft, Save, FolderTree, Loader2 } from 'lucide-react';
...
export default function EditCategoryPage() {
  ...
  const loadParentCategories = async () => {
    try {
      ...
      // Mevcut kategoriyi parent olarak seçilememesi için filtrele
  ...
  const handleNameChange = (value: string) => {
    setName(value);
    // Slug'ı sadece orijinal kategori slug'ı ile aynıysa otomatik güncelle
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (parentId === categoryId) {
      toast({
        title: 'Hata!',
        description: 'Kategori kendisini parent olarak seçemez.',
        variant: 'destructive'
      });
      return;
    }
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">Kategori yükleniyor...</p>
        </div>
      </div>
    );
  }
                  ...
                  <div className="space-y-2">
                    ...
                    {name && (
                      <div className="text-xs text-muted-foreground space-y-1">
                        ...
                        {category.displayName && category.displayName !== generateDisplayName(name) && (
                          <p className="text-blue-600">ℹ️ Mevcut görünür ad: {category.displayName}</p>
                        )}
                      </div>
                    )}
                  </div>
